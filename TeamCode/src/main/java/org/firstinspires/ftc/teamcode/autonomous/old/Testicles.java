...
@Autonomous(name="Testicles", group="FSMAuto", preselectTeleOp="teleOp")
@Disabled
public class Testicles extends LinearOpMode {
    ...
    Testicles.State currentState = Testicles.State.IDLE;
    Pose2d startPose = new Pose2d(-34.75, -62.75, Math.toRadians(-90));
    //Pose2d startPose = new Pose2d(-63, -38.7, Math.toRadians(0));Pose2d startPose = new Pose2d(-34.75, -62.75, Math.toRadians(270));
    ...
    public void runOpMode() throws InterruptedException {
        ...
        currentState = Testicles.State.SpikeDelivery;
        boolean set = false;
            ...
            switch (currentState) {
                ...
                case BackboardPixel1:
                    TrajectorySequence LeftPosBackboarddepart = drive.trajectorySequenceBuilder(poseEstimate)
                            ...
                            .lineToLinearHeading(new Pose2d(-60, -40, Math.toRadians(165))).build(); //-21, -60
                    if (!drive.isBusy()) {
                ...
                case testie1:
                    if (!drive.isBusy()) {
                        currentState = State.testie2;
                        TrajectorySequence PixelPickup222 = drive.trajectorySequenceBuilder(poseEstimate)
                                .setReversed(false)
                                .addTemporalMarker(0, () -> {
                                    claw.upperClaw(true);
                                    upperClawOpen = true;
                                ...
                                //    SampleMecanumDrive.getAccelerationConstraint(DriveConstants.MAX_ACCEL))
                                .build();
                        drive.followTrajectorySequenceAsync(PixelPickup222);
                    }
                    break;
                case testie2:
                    if (!drive.isBusy()) {
                        currentState = State.BackboardPixel4;
                        TrajectorySequence PixelPickup322 = drive.trajectorySequenceBuilder(poseEstimate)
                                .setReversed(true)
                                .addTemporalMarker(2.5, () -> {
                                    claw.upperClaw(false);
                                    upperClawOpen = false;
                                ...
                                .build();
                        drive.followTrajectorySequenceAsync(PixelPickup322);
                    }
                    break;
